...
StudentId: 50358236
Firstname Lastname: Ngoc Mai Nguyen
...
def add_friendship(network, name1, name2):
    ...
    for name in [ name1, name2 ]:
        if not name.isalpha():
            ...
            return
    network_keys = list(network.keys()) #list of all the names
    if name1 == name2: 
        return 
    else:
        if name1 not in network_keys:
            network[name1] = []
        if name2 not in network[name1]:
            network[name1].append(name2)
        if name2 not in network_keys:
            network[name2] = []
...
def print_command(network, namelist):
    ...
    if len(namelist) != 0:
        ...
        network_keys = list(network.keys()) #list of all the names
        for name in sorted(network_keys):
            print(str(name))
            friends = list(network[name])
...
def friends_command(network, namelist):
    ...
    if not name.isalpha():
        ...
        return
    network_keys = list(network.keys()) #list of all the names
    if name not in network_keys: #testing to see if the name from input is valid
        print(f"Error: '{name}' is an unknown name.")
        return
    friends = list(network[name]) #list of all friends that <name> has
    for friend in sorted(friends):
        print(str(friend))
...
def common_friends_command(network, namelist):
    ...
    for name in namelist:
        if not name.isalpha():
            ...
            return
    network_keys = list(network.keys()) #list of all the names
    common_friends = [] #list of commmon friends
    for name in namelist:
        if name not in network_keys:
            print(f"Error: '{name}' is an unknown name.")
            return
